...
v1.6.0 - Warnings continue
v1.7.0 - Warnings continue
...
#### Minor Version Upgrade (e.g., 0.1.0 → 0.2.0)
...
#### Major Version Upgrade (e.g., 0.x → 1.0)
...
### Upgrading from Alpha/Beta to Stable

When upgrading pre-release versions to v1.0.0:

1. **Review all release notes** from alpha/beta versions
2. **Check for API changes** in CLI and output format
...
```
v1.0.0 released: 2024-01-01
v2.0.0 released: 2025-01-01
v1.x bug fixes until: 2026-01-01
v1.x security patches until: 2026-07-01
```
...
func main() {
    if info, ok := debug.ReadBuildInfo(); ok {
        fmt.Println("wetwire-k8s-go version:", info.Main.Version)
    }
}
```
...
1. **Removed flags**: `--legacy-output` (use `--format yaml` instead)
2. **Changed defaults**: Default output format is now YAML (was JSON)
3. **Renamed commands**: `validate-schema` → `validate`

**Migration steps:**

```bash
# Before
wetwire-k8s build --legacy-output

# After
wetwire-k8s build --format yaml

# Before
wetwire-k8s validate-schema
...
1. **Check versions**: Verify wetwire-k8s-go, k8s.io/api, and cluster versions
2. **Review release notes**: Check for known issues
3. **Search existing issues**: [GitHub Issues](https://github.com/lex00/wetwire-k8s-go/issues)
4. **Report new issue**: Include version information

Issue template:

```markdown
**wetwire-k8s-go version**: 1.0.0
**k8s.io/api version**: v0.30.0
**Kubernetes cluster version**: 1.30.1
**Go version**: 1.22.0
**OS/Arch**: Linux/amd64
...
- Check [CODEGEN.md](CODEGEN.md) for updating k8s.io/api dependencies
- Review [DEVELOPERS.md](DEVELOPERS.md) for contributing
- See [CLI.md](CLI.md) for version-specific command reference
